...
tx.commit()
print(json.dumps(graph.data("match (u1:User)-[:posts]->(t:Tweet)-[:mentions]->(u2:User) where u2.screen_name={name} return u2.screen_name as User_Mention,u1.screen_name as User_Mentioning_User_Mention,count(t.id) as count order by count desc limit 2",{'name':'BJP4India'})))

def query1(author_screen_name):
	ans=json.dumps(graph.data("match (u1:User)-[:posts]->(t:Tweet)-[:mentions]->(u2:User) where u2.screen_name={name} return u2.screen_name as User_Mention,u1.screen_name as User_Mentioning_User_Mention,count(t.id) as count order by count desc limit 2",{'name':author_screen_name}));
	return ans

def query2(author_screen_name):
	ans=json.dumps(graph.data("match (u1:User)-[:posts]->(t:Tweet)-[:replies]->(t1:Tweet)-[:by]->(u2:User) where u2.screen_name={name} return u2.screen_name as User1,t1.id as Tweet_id_of_tweet_posted_by_user1,u1.screen_name as User_Replying_to_this_Tweet,collect(t.id) as Tweet_id_of_the_replies,count(t.id) as count order by count desc",{'name':author_screen_name}));
	return ans
